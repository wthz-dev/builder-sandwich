  ...
  <teleport to="body">
    <div v-show="open" class="fixed inset-0 z-50 grid place-items-center">
      ...
      <div class="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
        <button class="absolute right-3 top-3 rounded-full p-2 text-slate-600 hover:bg-slate-100" @click="$emit('close')" aria-label="ปิด">
        ...
        <div class="mt-4 grid place-items-center rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        <div class="mt-4 space-y-1 text-center text-xs text-slate-500">
          ...
          <p>เลขคำสั่งซื้อ: <span class="font-mono">{{ orderId }}</span></p>
        </div>
        <div class="mt-6 grid grid-cols-2 gap-2">
          <button class="rounded-xl border border-slate-200 px-4 py-2 text-slate-700 hover:bg-slate-50" @click="$emit('close')">ปิด</button>
          <button class="rounded-xl bg-slate-900 px-4 py-2 font-semibold text-white hover:bg-slate-800" @click="$emit('paid')">ฉันชำระแล้ว</button>
        </div>
      </div>
    </div>
  </teleport>
